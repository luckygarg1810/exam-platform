...
export const TeacherExamManage: React.FC = () => {
    ...
    const handleDelete = async () => {
        try { await deleteExam(eid); toast.success('Exam deleted'); navigate('/teacher') }
        catch (e: any) { toast.error(e.response?.data?.message || 'Failed') }
    }
    ...
    const tabs: { id: Tab; label: string }[] = [
        ...
        { id: 'proctors', label: 'Invigilators' },
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6 mb-5 animate-fade-in-up">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {exam.status === 'DRAFT' && (
                            <Button size="sm" onClick={handlePublish}>Publish Exam</Button>
                        )}
                        <div title={exam.status !== 'DRAFT' ? 'Exam can only be edited when DRAFT' : undefined}>
                            <Button size="sm" variant="secondary" disabled={exam.status !== 'DRAFT'} onClick={() => setShowEdit(true)}>Edit</Button>
                        </div>
                        {exam.status === 'ONGOING' && (
                            <Button size="sm" variant="secondary" onClick={() => navigate(`/teacher/exams/${eid}/invigulate`)}>Live Monitor</Button>
                        )}
                        <div title={isCompleted || exam.status === 'ONGOING' ? 'Exam cannot be deleted' : undefined}>
                            <Button size="sm" variant="danger" disabled={isCompleted || exam.status === 'ONGOING'} onClick={() => setShowDelete(true)}>Delete</Button>
                        </div>
                    </div>
                </div>
            </div>
